...
const (
	...
	FieldMtimeAfter    = "mtime_after"
	FieldMtimeBefore   = "mtime_before"
	FieldExcludeHidden = "exclude_hidden"
	FieldDedupe        = "dedupe"
...
type SearchParams struct {
	...
	PatternMode   string   `json:"pattern_mode"`             // Pattern mode: "normal", "regex", "extended_regex", "wildcard", "terms"
	...
	Scope         string   `json:"scope,omitempty"`          // Restrict results to this directory
	Exclude       []string `json:"exclude,omitempty"`        // Drop paths matching any of these globs
	Types         []string `json:"types,omitempty"`          // File extension filters (e.g. "go")
	MinSize       int64    `json:"min_size,omitempty"`       // Min file size in bytes
	MaxSize       int64    `json:"max_size,omitempty"`       // Max file size in bytes
	MtimeAfter    int64    `json:"mtime_after,omitempty"`    // Modified after (Unix seconds)
	MtimeBefore   int64    `json:"mtime_before,omitempty"`   // Modified before (Unix seconds)
	ExcludeHidden bool     `json:"exclude_hidden,omitempty"` // Exclude dotfiles
	Dedupe        bool     `json:"dedupe,omitempty"`         // Collapse hard links to one result
}
...
type Request struct {
	...
	PatternMode   string   `json:"pattern_mode"`             // Pattern mode: "normal", "regex", "extended_regex", "wildcard", "terms" (optional, for search)
	...
	Scope         string   `json:"scope,omitempty"`          // Restrict results to this directory (optional, for search)
	Exclude       []string `json:"exclude,omitempty"`        // Drop paths matching any of these globs (optional, for search)
	Types         []string `json:"types,omitempty"`          // File extension filters (optional, for search)
	MinSize       int64    `json:"min_size,omitempty"`       // Min file size in bytes (optional, for search)
	MaxSize       int64    `json:"max_size,omitempty"`       // Max file size in bytes (optional, for search)
	MtimeAfter    int64    `json:"mtime_after,omitempty"`    // Modified after, Unix seconds (optional, for search)
	MtimeBefore   int64    `json:"mtime_before,omitempty"`   // Modified before, Unix seconds (optional, for search)
	ExcludeHidden bool     `json:"exclude_hidden,omitempty"` // Exclude dotfiles (optional, for search)
	Dedupe        bool     `json:"dedupe,omitempty"`         // Collapse hard links to one result (optional, for search)
...
// ProtocolVersion is the wire protocol version. Bumped when request/response
// fields or method sets change incompatibly; exposed through the status RPC
// so clients can detect mismatched daemons.
